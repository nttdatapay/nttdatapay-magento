...
namespace Ndps\Aipay\Controller;
...
/**
 * Ndps Aipay Abstract Controller
 */
abstract class CfAbstract extends Action
{
    ...
    protected function validateWebhook($request, $order)
    {
        ...
        if ($orderAmount != $cfOrderAmount)
        {
            ...
            $this->logger->info(__("Ndps Error: ".$error_message));
        ...
        if (null !== $cfOrder && !empty($cfOrder[0]->payment_status))
        {
            ...
            if($cfOrderStatus === 'SUCCESS') {
                if($orderAmount === $cfOrderAmount) {
                    ...
                    $this->logger->info(__("Ndps Success: ".$transaction_message));
                } else {
                    ...
                    $this->logger->info(__("Ndps Error: ".$transaction_message));
                }
                return $validation_content;
            } else {
                $validation_content['txMsg'] = $transaction_message;
            ...
            $transaction_message = "Ndps Error: Order does not found. Please contact to merchant for support.";
